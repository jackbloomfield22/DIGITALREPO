    ...
    <div>
      <div className="mb-1 flex flex-wrap items-baseline justify-between gap-2">
        <div className="flex items-baseline gap-3">
          <h1 className="font-display text-3xl font-bold tracking-tight">YOUTUBE CHANNELS</h1>
          <span className="text-sm text-muted">{total}</span>
        </div>
        {canEdit && (
          <Link href="/youtube/new" className="btn btn-primary btn-sm">+ Add Channel</Link>
        )}
      </div>
      <p className="mb-5 max-w-2xl text-sm text-muted">
        The athlete channels business — who we&apos;d like to work with, who&apos;s signed,
        what&apos;s live, and the ideas queued up for each one. Distinct from{" "}
        <Link href="/formats" className="underline underline-offset-2 hover:text-accent-deep">Formats</Link>,
        which are single shows: a channel is the place a run of them lives.
      </p>

      <div className="mb-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        <Stat label="Channels live" value={String(live)} sub={`${total} in the business`} />
        <Stat label="Subscribers" value={compactNumber(reach._sum.subscribers ?? 0)} sub="across every channel" />
        <Stat label="Total views" value={compactNumber(reach._sum.totalViews ?? 0)} sub="as last checked" />
        <Stat label="Ideas in flight" value={String(ideasInFlight)} sub="queued across the slate" />
      </div>
