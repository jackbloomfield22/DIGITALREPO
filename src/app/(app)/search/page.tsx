              ...
              <section key={g.type} className="mb-6">
                ...
                <div className="divide-y divide-line overflow-hidden rounded-lg border border-line bg-surface">{g.items.map((item) => (
                  <Link key={item.id} href={item.href} className="block px-4 py-3 hover:bg-wash/60">
                    <div className="flex flex-wrap items-baseline justify-between gap-2"><span className="flex items-baseline gap-2"><span className="w-16 shrink-0 text-xs uppercase tracking-wide text-faint">{typeLabel(item.type)}</span><span className="break-words font-semibold">{item.label}</span></span><span className="text-xs text-muted">{item.unverified && !item.archived && <span className="mr-1.5 rounded bg-warn-wash px-1 py-0.5 text-xs font-semibold uppercase tracking-wide text-warn">Unverified</span>}{item.archived ? "Archived" : item.sub}{item.updatedAt ? ` · ${relativeTime(item.updatedAt)}` : ""}</span></div>
