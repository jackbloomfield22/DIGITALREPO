...
export type BriefTask = { id: string; title: string; kind: string; priority: number; dueAt: Date | null; relationshipName?: string | null; pipelineTitle?: string | null };
...
export type FocusItem = { score: number; why: string; kind: "task" | "follow_up" | "event" | "pipeline" | "relationship"; id: string; title: string; href: string };
...
export type Brief = {
  ...
  resurface: BriefIdea[];
  counts: { openTasks: number; activeCards: number; relationships: number; ideas: number };
};
...
export function buildBrief(
  input: { tasks: BriefTask[]; events: BriefEvent[]; pipelines: BriefPipeline[]; relationships: BriefRelationship[]; ideas: BriefIdea[] },
  now = new Date(),
): Brief {
  ...
  for (const e of eventsToday) if (e.pipelineTitle || e.relationshipName) focus.push({ score: 85, why: `today${e.allDay ? "" : ` at ${e.startsAt.toTimeString().slice(0, 5)}`} — prep${e.pipelineTitle ? ` for ${e.pipelineTitle}` : ""}${e.relationshipName ? ` with ${e.relationshipName}` : ""}`, kind: "event", id: e.id, title: e.title, href: "/hq/calendar" });
  for (const e of eventsTomorrow) if (e.pipelineTitle) focus.push({ score: 60, why: `tomorrow — prep for ${e.pipelineTitle}`, kind: "event", id: e.id, title: e.title, href: "/hq/calendar" });
