...
export type TaskRow = {
  id: string; title: string; kind: string; status: string; priority: number; dueAt: string | null; notes: string | null;
            ...
            <li key={t.id} className={`flex items-start gap-2.5 py-2 ${t.status === "done" ? "opacity-50" : ""}`}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap gap-x-2 text-xs text-faint">
                  {t.kind === "follow_up" && <span className="text-muted">follow-up</span>}
              ...
              {due.text && <span className={`shrink-0 text-xs ${due.tone}`}>{due.text}</span>}
